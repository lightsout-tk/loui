...
if(!function_exists( 'wpcast_template_post_grid' )){
	function wpcast_template_post_grid( $atts = array() ){
		...
		if ( $wp_query->have_posts() ) : 
			...
			?>
			<div id="<?php echo esc_attr( $grid_id ); ?>" class="wpcast-container wpcast-post-grid">
...
if(function_exists('ttg_custom_shortcode')) {
	ttg_custom_shortcode("qt-post-grid","wpcast_template_post_grid");
}
 ...
 */
add_action( 'vc_before_init', 'wpcast_template_post_grid_vc' );
if(!function_exists('wpcast_template_post_grid_vc')){
	function wpcast_template_post_grid_vc() {
  		vc_map( 
  			array(
				"name" => esc_html__( "Post grid", "wpcast" ),
				"base" => "qt-post-grid",
				"icon" => get_theme_file_uri( '/inc/ttgcore-setup/theme-functions/img/qt-post-grid.png' ),
				"description" => esc_html__( "Post grid", "wpcast" ),
